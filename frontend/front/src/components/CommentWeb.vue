<template>
  <v-col>
    <v-card :class="commentWidth" v-for="item in commentList" :key="item.ID" outlined>
      <v-row no-gutters v-if="commentList.length!==0">
        <v-col cols="10">
          ...
          <v-avatar size="40" class="mx-5" color="grey">
            ...
            <v-chip :color="item.User.role === 1 ? 'pink':'blue'" label class="white--text">{{item.User.role === 1? '管理员' : '用户'}}</v-chip>
            <div class="ml-3" style="display: inline-block">{{item.User.username}}</div>
          </div>
...
export default {
  ...
  methods:{
    changeSize(){
      ...
      this.commentWidth = this.viewportWidth > 900 ? 'ml-5 mr-5':' '
      this.textareaWidth = this.viewportWidth > 900 ? 'ma-5':'mt-5 mb-5'
    },
    handleCommentSubmit() {
